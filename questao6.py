...
print('{0} exercício 6.0 {0}'.format(5 * '='))
...
print('{0} exercício 6.1 {0}'.format(5 * '='))
...
print(troca_caixa('amanda vieira'))
...
print('{0} exercício 6.2 {0}'.format(5 * '='))
...
def imprime_mes_por_extenso(data):
    ...
    mes_extenso = meses[int(mes)]  # meses[1]
    return f"{dia} de {mes_extenso} de {ano}"


print(imprime_mes_por_extenso('25/05/2006'))
# output: 23 de April de 2023
...
print('{0} exercício 6.3 {0}'.format(5 * '='))
...
print('{0} exercício 6.4 {0}'.format(5 * '='))


def eh_sortudo(numero):
...
def numeros_sortudos(limite_inferior=1, limite_superior=100000):
    ...
    for numero in range(limite_inferior, limite_superior+1):
        if eh_sortudo(numero):
            total_sortudos += 1
    return total_sortudos


print(numeros_sortudos(100, 1000))
...
print('{0} exercício 6.5 {0}'.format(5 * '='))


def telefone_valido(telefone):
    # Regra 1: Não pode haver dois dígitos consecutivos idênticos
    for i in range(1, len(telefone)):
        if telefone[i] == telefone[i - 1]:
            return False

    # Regra 2: A soma dos dígitos tem que ser par
    if sum(int(d) for d in telefone) % 2 != 0:
        return False

    # Regra 3: O último dígito não pode ser igual ao primeiro
    if telefone[0] == telefone[-1]:
        return False

    # Se passou por todas as regras, o telefone é válido
    return True
...
if __name__ == '__main__':
    main()
    print("\n%d Testes, %d Ok, %d Falhas: Nota %.1f" % (total, acertos, total-acertos, float(acertos*10)/total))
    if total == acertos:
