...
def comprar_frutas(morango=0, uva=0):
    ...
    preco_uva = 1.8 if uva <= 5 else 1.5

    total_sem_des = (morango * preco_morango) + (uva * preco_uva)

    if morango + uva > 8 or total_sem_des > 25:
        total_com_des = total_sem_des * 0.9
    else:
        total_com_des = total_sem_des

    return round(total_com_des, 2)
